...
def init_db():
    ...
    try:
        collection = client.get_or_create_collection(name="passport_final_v5", embedding_function=emb_func)
...
# ==========================================
# 3. HÀM TỰ ĐỘNG TÌM MODEL (SỬA LỖI 404)
# ==========================================
def get_working_model():
    try:
        # Liệt kê các model khả dụng cho API Key này
        models = [m.name for m in genai.list_models() if 'generateContent' in m.supported_generation_methods]
        # Ưu tiên Flash 1.5, sau đó là Pro, cuối cùng là bất kỳ cái nào có sẵn
        for m in models:
...
def get_ai_response(user_query):
    ...
    try:
        ...
        return f"Lỗi AI: {str(e)}", None, None, None
...
if user_input:
    ...
    with st.chat_message("assistant"):
        with st.spinner("Đang kết nối cơ sở dữ liệu..."):
            ...
            if m_used:
                st.caption(f"Đã chạy trên: {m_used}")
