...
def init_db():
    ...
    try:
        # Sử dụng collection mới để làm sạch dữ liệu cũ
        collection = client.get_or_create_collection(name="passport_final_stable", embedding_function=emb_func)
...
# ==========================================
# 3. HÀM CHỌN MODEL AN TOÀN (TRÁNH BẢN PRO)
# ==========================================
def get_flash_model():
    try:
        # Lấy danh sách model và CHỈ chọn bản Flash
        models = [m.name for m in genai.list_models() if 'generateContent' in m.supported_generation_methods]
        for m in models:
            if "1.5-flash" in m: return m
        return "models/gemini-1.5-flash" # Tên mặc định nếu không list được
    except:
        return "models/gemini-1.5-flash"

# ==========================================
# 4. XỬ LÝ AI (TỐI ƯU TOKEN)
# ==========================================
def get_ai_response(user_query):
    if collection is None: return "Dữ liệu chưa sẵn sàng.", None, None, None

    # Chỉ lấy 1 đoạn văn bản duy nhất để tiết kiệm Input Token
    ...
    # Prompt cực ngắn để không tốn quota
    prompt = f"Thông tin: {context}\n\nHỏi: {user_query}\nĐáp (ngắn gọn):"
...
if user_input:
    ...
    with st.chat_message("assistant"):
        with st.spinner("Đang tra cứu..."):
            ...
            if "429" in answer:
                full_res = "⚠️ Hệ thống đang tạm thời hết lượt dùng miễn phí. Vui lòng thử lại sau 60 giây."
            elif url:
            ...
            if m_used: st.caption(f"Model: {m_used}")
